...
export function setResolution(options) {
    document.querySelectorAll(".ChangeResolution .btn-container button").forEach(btn=> {
        btn.classList.remove('activeResolution');
        if (btn.getAttribute('res') === options.video_res) {
            btn.classList.add("activeResolution");
        }
    })
}
